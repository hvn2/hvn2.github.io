...
**Ánh xạ đảo:** $A: H_0 \rightarrow H_1$ khả đảo nếu tồn tại $B: H_1 \rightarrow H_0$ sao cho, với $x \in H_0$ ,và $y \in H_1$:
...
$$\langle Ax,y \rangle {H_1} = \langle x,A^*y \rangle {H_)}$$

Nếu $A=A^*$ thì gọi là ánh xạ tự chuyển vị (self adjoint)

**Ánh xạ đơn vị (unity):** $A: H_0 \rightarrow H_1$ là ánh xạ đơn vị nếu:
  - Khả đảo (invertible)
  - Giữ nguyên tích vô hướng: $\langle Ax,Ay \rangle {H_1} =\langle x,y \rangle {H_0}$
...
**Ánh xạ xác định dương (Positve definite):** Ánh xạ tự chuyển vị (sefl adjoint) $A: H_ \rightarrow H$ gọi là:
...
**Ánh xạ (toán tử) chiếu vuông góc (Orthogonal Projection):** Phép chiếu vuông góc một vector lên một không gian con tìm ra xấp xỉ tốt nhất của vector trên không gian con đó.
...
Phép chiếu cho phép phân tích vector $x$ thành hai thành phần xấp xỉ trên hai không gian con vuông góc với nhau. Như hình 1, $x = \hat{x}+(x-\hat{x})$ với $\hat{x}\in S$ và $x-\hat{x} \in S^{\perp}$. Đồng thời phép chiếu cũng cho phép xác định xấp xỉ của một vector lên một không gian con thuận tiện thông qua phép nhân ma trận.
...
$$Px =\langle x,\phi \rangle \phi$$

với $\phi \in \mathbb{R^1}$ (1 đường thẳng), là một phép chiếu vuông góc vì nó thỏa mãn 2 tính chất: $P(Px)=\langle{\langle x,\phi \rangle,y}\rangle = \langle x,\phi \rangle \langle \phi, \phi \rangle \langle x, \phi \rangle \phi$
