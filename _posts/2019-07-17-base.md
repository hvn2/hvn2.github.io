...
Nếu $\mathbf{x,y} \in W$ thì $\mathbf{x+y} \in W$ và $\alpha\mathbf{x} \in W$  **Ví dụ:** Trong không gian vector các số thực 2 chiều $\mathbf{R^2}$ thì tập hợp các điểm nằm trên một đường thẳng đi qua gốc tọa độ là một không gian con. Không gian con $\mathbf{W}$ có số chiều nhỏ hơn hoặc bằng không gian mẹ $\mathbf{V}$.
