...
- **Hệ sinh (span)**: Tập hợp tất cả các tổ hợp tuyến tính của các vector của $S$ gọi là hệ sinh bởi $S$ hay $span(S)$.
    ...
    + Nếu $S$ là một cơ sở bất kỳ thì người ta luôn tìm được một cơ sở gọi là dual basis với nó $S'$ sao cho $\langle\mathbf{a_i,a'_k}\rangle = \delta_{i-k}$ (bằng 0 nếu $i\neq k$ và bằng 1 nếu $i=k$). $S$ và $S'$ tạo thành một biorthogonal basis.
