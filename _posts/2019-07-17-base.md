...
**Ánh xạ tuyến tính (Linear Operator):** Một hàm số $A: H_0 \rightarrrow H_1$ là một AXTT từ $H_0 \rightarrow H_1$ nếu nó thỏa mãn các tính chất của phép cộng và nhân với một hằng số:
...
**Ánh xạ chuyển vị (Adjoint):** $A*: H_1 \rightarrow H_0$ gọi là ánh xạ chuyển vị của $A: H_0 \rightarrow H_1$ nếu:

$$\langle Ax,y \rangle_{H_1} = \langle x,A*y \rangle_{H_)}&&

Nếu $A=A*$ thì gọi là ánh xạ tự chuyển vị (self adjoint)

**Ánh xạ đơn vị (unity):** $A: H_0 \rightarrow H_1$ là ánh xạ đơn vị nếu:
  - Khả đảo (invertible)
  - Giữ nguyên tích vô hướng: $\langle Ax,Ay \rangle_{H_1} =\langle x,y \rangle_{H_0}$
